...
private:
    ...
    std::map<unsigned int, std::vector<std::pair<PrintStateBase::Warning, size_t>>> current_warnings;    bool show_warning_dialog { false };
...
unsigned int Plater::priv::update_background_process(bool force_validation, bool postpone_error_messages, bool switch_print)
{
    ...
    if ((invalidated != Print::APPLY_STATUS_UNCHANGED || force_validation) && ! background_process.empty()) {
        ...
        if (err.string.empty()) {
            ...
            notification_manager->set_all_slicing_errors_gray(true, partplate_list.get_curr_plate_index());            notification_manager->close_notification_of_type(NotificationType::ValidateError);
        ...
        for (auto const& warning : state.warnings) {
            if (warning.current) {
                ...
                if (evt.status.message_type == PrintStateBase::SlicingNotificationType::SlicingReplaceInitEmptyLayers ||
                    ...
                    evt.status.message_type == PrintStateBase::SlicingNotificationType::SlicingNotEnoughFilament) {                    notif_level = NotificationManager::NotificationLevel::SeriousWarningNotificationLevel;
                }
